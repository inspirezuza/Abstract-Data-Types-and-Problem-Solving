...
int search(tree_t *t, int node_data) {
    ...
    return search(t->first_child, node_data) ||
           search(t->next_sibling, node_data);
}
...
// 11
int ancestor(tree_t *t, int node_data) { print_path(t, t->value, node_data); }
...
void print_tree(tree_t *t) { print_tree_helper(t, 0); }
